...
Dari Tabel 4.1 terlihat bahwa jumlah pelanggan PLN UP3 Malang mengalami tren pertumbuhan yang konsisten positif selama periode 24 bulan, dengan rata-rata pertumbuhan bulanan sekitar 3.762 pelanggan baru per bulan. Pertumbuhan tertinggi terjadi pada bulan September 2024 (5.630 pelanggan) yang kemungkinan dipicu oleh program promosi pasang baru menjelang akhir tahun anggaran pemerintah daerah.

#figure(
  image("../../images/grafik-pelanggan.jpg", width: 100%),
  kind: "gambar",
  caption: [Grafik Jumlah Pelanggan PLN UP3 Malang (Januari 2024 -- Desember 2025)],
)
...
Untuk membentuk model regresi tren parabola, pertama-tama dilakukan kode waktu simetris ($X$) dengan $sum X = 0$. Karena jumlah data genap ($n=24$), maka nilai $X$ berupa bilangan bulat tanpa nilai 0: -12; -11; ...; -1; 1; ...; 11; 12.
...
#figure(
  table(
    ...
    [$sum X^2$], [1.300,00], [Jumlah kuadrat waktu],
    [$sum X^4$], [121.420,00], [Jumlah pangkat empat waktu],
    [$sum Y$], [33.057.195], [Total keseluruhan jumlah pelanggan],
    [$sum X Y$], [4.824.412,00], [Jumlah perkalian X dan Y],
    [$sum X^2 Y$], [1.790.512.508,00], [Jumlah perkalian $X^2$ dan Y],
  ),
  kind: "tabel",
...
#figure(
  table(
    ...
    [*$a$ (Konstanta)*], [1.377.473,99], [$ ((sum Y)(sum X^4) - (sum X^2 Y)(sum X^2)) / (n(sum X^4) - (sum X^2)^2) $], [Estimasi jumlah pelanggan pada titik tengah periode (antara Des 2024-Jan 2025)],
    [*$b$ (Koefisien Linear)*], [3.711,09], [$ sum X Y / sum X^2 $], [Kontribusi linear waktu terhadap pertumbuhan pelanggan (positif = tren naik)],
    [*$c$ (Koefisien Kuadratik)*], [(-1,68)], [$ (n(sum X^2 Y) - (sum X^2)(sum Y)) / (n(sum X^4) - (sum X^2)^2) $], [Faktor kelengkungan kurva (negatif = kurva terbuka ke bawah, laju pertumbuhan menurun)],
  ),
  kind: "tabel",
...
*Interpretasi Model:*
+ *Konstanta ($a = 1.377.473,99$):* Pada titik tengah periode observasi (antara Desember 2024 dan Januari 2025, di mana $X=0$), estimasi jumlah pelanggan PLN UP3 Malang adalah sekitar *1,38 juta pelanggan*. Nilai ini sangat mendekati rata-rata aktual (1.377.383), yang memvalidasi keakuratan model.
+ *Koefisien Linear ($b = 3.711,09$):* Setiap kenaikan 1 satuan waktu (1 bulan), jumlah pelanggan akan meningkat rata-rata sebesar *3.711 pelanggan* secara linear. Nilai positif mengkonfirmasi tren pertumbuhan yang konsisten naik.
+ *Koefisien Kuadratik ($c = -1,68$):* Nilai negatif menunjukkan bahwa kurva parabola *terbuka ke bawah* (*concave*), yang berarti laju pertumbuhan pelanggan cenderung *sedikit menurun* (*decelerating growth*) seiring berjalannya waktu. Meskipun jumlah pelanggan tetap bertambah setiap bulan, besar pertambahannya semakin mengecil. Hal ini dapat mengindikasikan bahwa pasar pelanggan listrik di wilayah Malang mulai mendekati titik jenuh (*market saturation*).
...
$ R^2 = 1 - (sum (Y_i - hat(Y)_i)^2) / (sum (Y_i - bar(Y))^2) = 0,9963 "atau" bold("99,63%") $

*Interpretasi:*
- Nilai $R^2 = 99,63$\% menunjukkan bahwa *model regresi tren parabola yang dibentuk mampu menjelaskan 99,63\% variasi atau perubahan jumlah pelanggan PLN UP3 Malang* selama periode 2024-2025.
- Hanya *0,37\%* variasi data yang tidak dapat dijelaskan oleh model, yang kemungkinan disebabkan oleh faktor-faktor stokastik musiman (seperti jumlah hari kerja per bulan, libur nasional, atau kampanye promosi spesifik).
...
#figure(
  table(
    ...
    [Jan 2024], [-12], [1.333.762], [1.332.699], [+1.063], [+0,08\%],
    [Jun 2024], [(-7)], [1.349.742], [1.351.414], [(-1.672)], [(-0,12\%)],
    [Des 2024], [(-1)], [1.377.216], [1.373.761], [+3.455], [+0,25\%],
    [Jun 2025], [7], [1.398.106], [1.400.336], [(-2.230)], [(-0,16\%)],
    [Des 2025], [12], [1.424.059], [1.421.765], [+2.294], [+0,16\%],
  ),
  kind: "tabel",
...
Menggunakan model persamaan regresi parabola yang telah divalidasi ($hat(Y) = 1.377.473,99 + 3.711,09 X - 1,68 X^2$), dilakukan peramalan jumlah pelanggan untuk periode Januari -- Desember 2026. Untuk tahun 2026, nilai $X$ dilanjutkan dari kode waktu sebelumnya: dimulai dari $X = 13$ (Januari 2026) hingga $X = 24$ (Desember 2026).

#figure(
  table(
    columns: (auto, auto, auto, auto, auto, auto, auto),
    [*No*], [*Periode*], [*Nama Bulan*], [*$X$*], [*Prediksi Jumlah Pelanggan ($hat(Y)$)*], [*Pertumbuhan Bulanan*], [*Pertumbuhan Kumulatif (dari Des 2025)*],
    [1], [2026-01], [Januari], [13], [1.425.435], [3.669], [1.376],
    [2], [2026-02], [Februari], [14], [1.429.100], [3.666], [5.041],
    [3], [2026-03], [Maret], [15], [1.432.763], [3.662], [8.704],
    [4], [2026-04], [April], [16], [1.436.422], [3.659], [12.363],
    [5], [2026-05], [Mei], [17], [1.440.078], [3.656], [16.019],
    [6], [2026-06], [Juni], [18], [1.443.730], [3.652], [19.671],
    [7], [2026-07], [Juli], [19], [1.447.379], [3.649], [23.320],
    [8], [2026-08], [Agustus], [20], [1.451.025], [3.646], [26.966],
    [9], [2026-09], [September], [21], [1.454.667], [3.642], [30.608],
    [10], [2026-10], [Oktober], [22], [1.458.306], [3.639], [34.247],
    [11], [2026-11], [November], [23], [1.461.942], [3.636], [37.883],
    [12], [2026-12], [Desember], [24], [*1.465.574*], [3.632], [*41.515*],
  ),
  kind: "tabel",
...
- *Proyeksi Jumlah Pelanggan Akhir 2026:* *1.465.574 pelanggan* (naik 2,92\% dari Des 2025)
- *Total Pertumbuhan Tahun 2026:* *41.515 pelanggan baru* (rata-rata *3.460 pelanggan/bulan*)
- *Laju Pertumbuhan:* Model memprediksi bahwa laju pertumbuhan bulanan akan sedikit menurun dari ~3.669 di awal 2026 menjadi ~3.632 di akhir 2026, konsisten dengan nilai koefisien kuadratik negatif ($c=-1,68$) yang mengindikasikan pertumbuhan melambat (*decelerating growth*).
...
+ *Arah Tren Positif:* Koefisien linear ($b$) bernilai positif (3.711,09), mengkonfirmasi bahwa jumlah pelanggan terus bertambah setiap bulannya tanpa penurunan.
+ *Perlambatan Pertumbuhan:* Koefisien kuadratik ($c$) bernilai negatif (-1,68), menunjukkan bahwa laju pertumbuhan tidak konstan melainkan sedikit menurun seiring waktu. Artinya, meskipun jumlah pelanggan tetap bertambah, besar pertambahan bulanannya cenderung semakin mengecil. Hal ini dapat menjadi indikasi awal bahwa pasar pelanggan listrik di wilayah Malang mulai mendekati titik jenuh (*market saturation*).
+ *Stabilitas Tinggi:* Nilai $R^2$ yang mendekati sempurna (99,63\%) menunjukkan bahwa pertumbuhan pelanggan sangat teratur dan dapat diprediksi dengan tingkat akurasi tinggi.
...
+ *Perencanaan Infrastruktur:*
  - Dengan proyeksi tambahan ~41.500 pelanggan baru di tahun 2026, PLN perlu menyiapkan kapasitas gardu distribusi tambahan sebesar minimal 130-180 MVA (dengan asumsi rata-rata daya 3.000 VA/pelanggan).
...
+ *Perencanaan SDM:*
  - Pertumbuhan pelanggan ~3.700/bulan membutuhkan penambahan personel teknis (teknisi jaringan) dan admin pelayanan minimal 8-12 orang untuk menjaga rasio optimal.
...
Meskipun model regresi parabola menunjukkan performa sangat baik ($R^2=99,63$\% ), perlu dicatat beberapa keterbatasan:
